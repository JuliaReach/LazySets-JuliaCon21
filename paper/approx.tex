...
In two dimensions, LazySets can compute $\varepsilon$-close overapproximations via \code{overapproximate(X, $\varepsilon$)}.
...
The smallest bounding box, which is available via \code{overapproximate(X, Hyperrectangle)} (or the equivalent shorthand \code{box\_approximation(X)}), yields a \code{Hyperrectangle}, which is more efficient to work with.
%
We can also obtain a zonotope via \code{overapproximate(P, Zonotope, D)} where \code{P} is a polytope and \code{D} is a vector of directions used as candidates for the generators.
